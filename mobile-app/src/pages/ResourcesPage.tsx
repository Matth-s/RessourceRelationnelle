...
import { Search, SlidersHorizontal, Eye, Heart, X, Image, Film, FileText, Music } from "lucide-react";
...
const ResourcesPage = () => {
  const [resources, setResources] = useState<ResourceReturn[]>([]);
  ...
  const filtered = resources.filter((r) => {
    const matchSearch = r.title.toLowerCase().includes(search.toLowerCase()) || r.resume.toLowerCase().includes(search.toLowerCase());
    ...
    const matchType = selectedType ? r.typeResource?.typeRessource === selectedType : true;
      ...
      <main className="flex-1 px-4 py-6 md:py-10 max-w-6xl mx-auto w-full">
        ...
        <div className="flex gap-2 mb-4">
          ...
          <button
            ...
            className={`flex items-center gap-2 px-4 py-3 rounded-xl border text-sm relative ${showFilters ? "bg-blue-50 border-blue-200 text-blue-600" : "bg-white border-gray-200 text-gray-600"}`}
          >
            <SlidersHorizontal className="h-4 w-4" />
            <span className="hidden sm:inline">Filtres</span>
            {activeFiltersCount > 0 && (
              <span className="absolute -top-1.5 -right-1.5 bg-blue-600 text-white text-[10px] w-5 h-5 rounded-full flex items-center justify-center">{activeFiltersCount}</span>
            )}
          </button>
        </div>
...
const FilterChip = ({ label, active, onClick }: { label: string; active: boolean; onClick: () => void }) => (
  <button
    onClick={onClick}
    className={`px-2.5 py-1 rounded-full text-xs font-medium transition-colors ${active ? "bg-blue-600 text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`}
  >
    {label}
  </button>
);

const ResourceCard = ({ resource }: { resource: ResourceReturn }) => {
  ...
  const hasMedia = resource.mediaUrl && resource.mediaUrl.length > 0;
  ...
  return (
    <Link to={`/resources/${resource.id}`} className="bg-white rounded-xl border border-gray-100 overflow-hidden hover:shadow-md transition-shadow">
      ...
      {hasMedia && resource.mediaType === "image" ? (
      ...
      <div className="p-4">
        <div className="flex items-center gap-2 mb-2">
          {categoryName && (
            <span className={`text-xs font-medium px-2.5 py-1 rounded-full ${categoryColor}`}>{categoryName}</span>
          )}
          {typeName && (
            <span className="text-xs font-medium px-2.5 py-1 rounded-full bg-gray-100 text-gray-600">{typeName}</span>
          )}
        </div>
        ...
        <div className="flex items-center justify-between text-xs text-gray-400">
          <div className="flex items-center gap-3">
            <span className="flex items-center gap-1"><Eye className="h-3.5 w-3.5" />{resource.viewCount}</span>
            <span className="flex items-center gap-1"><Heart className="h-3.5 w-3.5" />{resource.likeCount}</span>
          </div>
