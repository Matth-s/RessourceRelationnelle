...
const ResourcesPage = () => {
  ...
  const filtered = resources.filter((r) => {
    ...
    const matchCategory = selectedCategory
      ? r.category?.categoryName === selectedCategory
      : true;
    const matchRelation = selectedRelation
      ? r.typeRelation?.typeRelation === selectedRelation
      : true;
    const matchType = selectedType
      ? r.typeResource?.typeRessource === selectedType
      : true;
    ...
    <div className="min-h-screen flex flex-col bg-gray-50">
      ...
      <main className="flex-1 px-4 py-6 md:py-10 max-w-6xl mx-auto w-full">
        ...
        <p className="text-gray-500 text-sm mb-6">
          Découvrez des centaines de ressources pour améliorer vos
          relations
        </p>
        ...
        {showFilters && (
          <div
            ...
            className="bg-white rounded-xl border border-gray-200 p-4 mb-4 space-y-4"
          >
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1.5">
                <FilterChip
                  label="Toutes"
                  active={selectedRelation === ''}
                  onClick={() => setSelectedRelation('')}
                />
                {typeRelations.map((rel) => (
                  <FilterChip
                    key={rel.id}
                    label={rel.typeRelation}
                    active={selectedRelation === rel.typeRelation}
                    onClick={() =>
                      setSelectedRelation(rel.typeRelation)
                    }
                  />
                ))}
              </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1.5">
                ...
                {typeResources.map((type) => (
                  <FilterChip
                    key={type.id}
                    label={type.typeRessource}
                    active={selectedType === type.typeRessource}
                    onClick={() =>
                      setSelectedType(type.typeRessource)
                    }
                  />
                ))}
              </div>
...
const ResourceCard = ({ resource }: { resource: ResourceReturn }) => {
  ...
  const categoryColor =
    CATEGORY_COLORS[categoryName] ?? 'bg-gray-100 text-gray-700';
