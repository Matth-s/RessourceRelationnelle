    ...
    <header className="bg-white border-b border-gray-100 sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-4 flex items-center justify-between h-16">
        ...
        <nav className="hidden md:flex items-center gap-6">
          ...
          <Link to="/resources" className="text-sm text-gray-600 hover:text-gray-900">
            Ressources
          </Link>
      ...
      {menuOpen && (
        <nav className="md:hidden bg-white border-t border-gray-100 px-4 py-4 space-y-3">
          <Link to="/" className="block text-sm text-gray-600" onClick={() => setMenuOpen(false)}>
            Accueil
          </Link>
          <Link to="/resources" className="block text-sm text-gray-600" onClick={() => setMenuOpen(false)}>
            Ressources
          </Link>
          <Link to="/help" className="block text-sm text-gray-600" onClick={() => setMenuOpen(false)}>
            Aide
          </Link>
          ...
          {user ? (
            ...
            <>
              <Link to="/auth/login" className="block text-sm text-gray-600" onClick={() => setMenuOpen(false)}>
                Connexion
              </Link>
              <Link
                to="/auth/register"
